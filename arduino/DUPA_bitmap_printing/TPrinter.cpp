...
void Tprinter::printBitmap(uint8_t *bitmap, uint16_t width, uint16_t height, uint8_t scale, bool center) {
  uint16_t maxScale = widthInDots / width;  // widthInDots = 384;
  if (scale == 0 || scale > maxScale) scale = maxScale;

  bool isMarginAdded = {};
  const uint16_t marginWidth = center ? (widthInDots - scale * width) / 2 : 0;
  uint16_t marginCounter = {};

  const uint16_t bytesPerRow = (scale * width + marginWidth + 7) / 8;

  const uint16_t bufferSize = (printerBufferLimit / bytesPerRow) * bytesPerRow;
  uint16_t bufferFillLevel = {};
